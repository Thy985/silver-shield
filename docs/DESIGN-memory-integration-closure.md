...
**答（设计）**：建立至少 1 个走**真实 detector→tracker→event_builder→rule→decision→memory** 的场景（非 StubDetector）。验收：视频/帧输入 → 风险事件 → EpisodicRecord，且 `source_event_ids` 能追溯到真实 visitor/warning/action。真实输入的最小形态见 §3.7。
...
> 这条与 §3.1 一致：目标是"链路真实可运行"，不是"检测更准"。fixture 小、可提交、torch-free 也能跑通整链。
...
### Slice A — Memory Runtime Integration（运行时集成结构化）

**目标**：Memory 在 runtime 中从"内联 `if episodic_shadow: _record_episode(...)`"整理为清晰的 **Memory Hook** 结构：
...
- 明确接线契约（Memory Hook 的输入是 `VisitorEvent + warnings + actions`，输出是 `episodes_recorded` / `errors` 指标）。
- **纪律**：`MemoryPolicy.project_episode` / `EpisodicRecord` / `VisitorEvent` 签名**一律不动**（多模态泛化不在本阶段）。

**验收**：
- `memory.enabled=False` 完全不影响主链；
- Memory 异常不影响风险判断（沿用 E2E-4 守护）；
- latency 增量可控（旁路 O(n)，宽松上界守护）。

**说明**：本 Slice 功能已基本存在（Stage F），重点是**结构整理 + 文档化接线契约 + 避免主链污染**，不是新功能、不改接口。

### Slice B — Closed Loop Scenario Test（闭环场景测试）

系统级场景（torch-free 优先；真实输入场景见 §3.7）：

| 场景 | 输入 | 输出 | 验收重点 |
|---|---|---|---|
| **场景 1 陌生访客踩点** | enter→dwell→leave（**真实 detector 路径**：最小 fixture `stranger_visit_short.mp4` + 缓存检测） | WarningEvent + EpisodeRecord | 事件 ID 一致（`source_event_ids` 引用 warning/action）；链路真实跑通 |
...
**注（Data Flow Closure 真实输入）**：场景 1 必须走**真实 detector→tracker→event_builder**，但用 §3.7 的**最小 fixture + 缓存检测**，不碰 CAVIAR。fixture 进 `tests/fixtures/`（小体积、可提交、torch-free 可跑整链）。此 fixture 在 Slice B 开工时建立。

### Slice C — Memory Evaluation + Product Closure（评估 + 用户价值验收）★ 最高优先

在 ADR-0024 Slice 6 评估基础上扩展，并把 **Product Closure（§3.6）作为本 Slice 的硬交付**：

1. **信息损失评估（系统级）**：Raw Event 10000 条 → 1 条 Episode，关键字段保持：`visitor / time / risk / action / evidence`。补"跨阶段信息溯源"——从 Episode 反查能定位到哪条 WarningEvent / ActionCommand。
2. **Replay 稳定性（版本矩阵）**：不同 Memory 版本（v1 / v2）同输入结果一致（沿用 E2E-3，本 Slice 加版本维度）。
3. **用户价值验收 / Product Closure（核心）**：实现轻量 `MemoryQuery.compose_context(visitor, window)` 原型（**不实现 Agent、不接 LLM**），产出 §3.6 的 JSON：
   ```
   返回 {
     current_status:  ACTIVE_RISK | CLEARED | None,
     reason:          str,                         # 由 Episode + Warning 组合生成
     evidence:        [str…],                      # 证据链（可溯源到 record/warning）
     handling:        str,                         # 行动记录
     history:         str,                         # 历史事件汇总
   }
   ```
   **验收（不可妥协）**：该 JSON 必须由 Memory 真实存储的 `EpisodicRecord` + `WarningEvent` 组合生成，字段来源可追溯到具体 record/warning；并有一个断言"给定固定 visitor + 固定 window，输出 JSON 稳定可重放"。这证明 Memory 不是内部正确而已，而是**真能产生用户可理解的价值**。
