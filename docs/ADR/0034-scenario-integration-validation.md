...
- **2026-08-12（v1.0 实现冻结 · Owner 授权）**：三阶段实现全部合入 `main`，按用户验收口径（C1–C8）逐条核对通过，本 ADR 达 v1.0。
  - **Phase A**（PR #180 → `ae5ce51`）：单模态最小闭环（Scenario→Runtime→Decision→Notification）+ Memory 落库断言 + `ActionSink` 三件套（G1）+ `IntegrationContext`/`IntegrationRunner`（L1–L5，签名冻结 `run(scenario, context=None)`）。
  - **Phase B**（#181/#182/#183/#184 → `f482b2b`）：Memory 深度期望 + 跨模态真实闭环（G3 显式注入 / G4 薄 Adapter `bridge.py`）+ 两枚闭环指纹（`expectation_fingerprint` ⊂ `loop_fingerprint`，G7）。
  - **Phase C**（#185 → `890c808`；DoD 补全 #187 → `d4cc391`；治理修复 #188 → `454345d`；基线环境对齐 #190；测试覆盖补全 #191）：per-expectation severity（blocking/warning，F6 恒 blocking 不可降级）+ `evaluate_integration_gate`（`gate.passed` blocking 语义）+ CI `integration-gate` job（消费 `IntegrationReport` 决策，非 pytest exit code）+ loop 指纹基线漂移治理（`integration-baseline-bump` 标记，类比 ADR-0033 `benchmark-baseline-bump`）。
  - **DoD 验收（C1–C8）**：C1 独立 `integration-gate` job 自动触发 ✅｜C2 CI 按 `gate.passed` + gate.json（passed/degraded/verdicts(failure_code)）决策 ✅｜C3 severity 分级生效（t10/t17/t18/t19 测试，warning 降级可见于 artifact）✅｜C4 两枚指纹接入基线漂移检测（合法更新须标记，非预期 FAIL）✅｜C5 artifact 全量上传（summary/fingerprints/canonical/gate + junit/coverage）✅｜C6 F2/F3/F5 failure injection 测试入 CI integration tier（#191）✅｜C7 环境可复现（Python 3.12 + numpy 2.4.2 / opencv 4.13.0.92 钉死 + provenance 报告 code_version/python/numpy/opencv/torch）✅｜C8 T2 生产边界 AST 契约入 CI（`test_t2_production_does_not_import_loop_package` + `test_adr0033_t2_evaluation_not_wired_into_production`）✅。
  - 实现级细节（PR 对应关系 / 验收证据）见 `0034-implementation-plan.md` 与项目工作日志；本条目按 AGENTS.md §6.3 由 Owner 授权落库。
