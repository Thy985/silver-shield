...
- **2026-08-04** 评审后修订（review-ready）：
  ...
  - **二轮 Spike 评审修订（2026-08-04）**：① §9 Phase 3.0 拆 **P0（`FileAudioSource` 必须·CI 可测可复现零硬件）/ P1（`LocalMicSource` 增强·现场 Demo，延后以避免 Windows 音频设备非核心风险）**；② §8 补 Spike #2 结论——音频不应每视频帧同步调用，须独立 `Audio Loop` 经 event bus 异步消费；③ 新增配套文档指针：`docs/audio_stack_survey.md`（选型）+ `docs/audio_spike_report.md`（Spike 实证）；④ 调研文档同步将 YAMNet 运行时表述由"YAMNet ONNX 已确定"收紧为"**ONNX Runtime 为 py3.13 Spike 验证路径，YAMNet ONNX 模型转换与许可证/权重来源待实现阶段确认**"。
