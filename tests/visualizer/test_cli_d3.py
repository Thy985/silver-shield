...
def _args(tmp_path: Path, artifact_dir: Path, extra=None) -> list[str]:
    base = [
...
def test_cli_success_exit_0(tmp_path: Path, built_artifact_dir):
    rc = main(_args(tmp_path, built_artifact_dir))
    assert rc == 0
    case_mp4 = tmp_path / f"{_SCENARIO}__v1" / "case.mp4"
    assert case_mp4.exists() and case_mp4.stat().st_size > 0


def test_cli_provenance_g2_required_keys(tmp_path: Path, built_artifact_dir):
...
def test_cli_with_audio_exit_1(tmp_path: Path, built_artifact_dir):
    rc = main(_args(tmp_path, built_artifact_dir, extra=["--with-audio"]))
    assert rc == 1
...
def test_cli_with_audio_verbose_exit_1(tmp_path: Path, built_artifact_dir):
    # --verbose 不得让异常逃逸出 main()（仍可被单测捕获退出码）
    rc = main(_args(tmp_path, built_artifact_dir, extra=["--with-audio", "--verbose"]))
    assert rc == 1


def test_cli_missing_scenario_exit_1(tmp_path: Path, built_artifact_dir):
    rc = main([
        "--artifact-dir", str(built_artifact_dir),
...
def test_cli_bad_resolution_exit_2(tmp_path: Path, built_artifact_dir):
    rc = main([
        "--artifact-dir", str(built_artifact_dir),
        "--scenario-id", _SCENARIO,
        "--output-dir", str(tmp_path),
