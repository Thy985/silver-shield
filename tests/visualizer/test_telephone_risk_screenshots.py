...
SHOTS: tuple[tuple[str, str, str], ...] = (
    ("01-video-sensor.png", f"video-sensor-{SID}", "① 视频区（sensor-card 本体，覆盖 B6 样式检查面）"),
...
KNOWN_CONSOLE_EXEMPT: tuple[str, ...] = (
    ...
    # 不经 Playwright response 事件，仅 console 报错）。非产品缺陷。
    "favicon.ico",
)
...
# RAISED 未达成时的诊断快照（定位卡点：首帧/感知/行为/音频健康/风险空态文案）。
_DIAGNOSTICS_JS = """
(() => {
  const g = (id) => document.getElementById(id);
  const audioCard = g('audio-sensor-__SID__');
  const emptyCard = g('lrk-empty-__SID__');
  const psRecent = g('ps-recent-__SID__');
  const bt = g('behavior-timeline-__SID__');
  return {
    audioHealth: audioCard ? (audioCard.getAttribute('data-audio-health') || '') : 'NO_NODE',
    lrkEmptyText: emptyCard ? (emptyCard.textContent || '').trim() : 'NO_NODE',
...
# 连拍执行顺序（易逝的 ③/③.5 最先，稳定区域垫后）；manifest 仍按 SHOTS 冻结编号输出。
...
def _try_capture_burst(page: Any) -> list[dict[str, str]] | None:
    ...
    页面是活的（§7.8）：音频证据回落后 levels 变空，lrk-card 会被合法隐藏。
    故连拍序列把易逝的 ③风险解释卡 / ③.5风险信号 放最前，稳定区域垫后；
    中途失手返回 None 由调用方等待下一个 RAISED 窗口重试。
    """
    ...
    for fname, dom_id, desc in sorted(
        ...
        except Exception:  # noqa: BLE001 — 连拍中途页面变化，整体重试
            return None
...
def _browser():
    """单真实 Browser Session（headless Chromium，1440x900 viewport，同 D0）。"""
...
@pytest.fixture(scope="module")
def risk_session(_browser):
    """risk 主场景就绪 → RAISED 可见窗口内事件驱动连拍（活页面竞态对策）。"""
    page = _browser["page"]
    ...
    if not raised_ready:
        ...
        except Exception:  # noqa: BLE001 — 诊断失败不掩盖主断言
...
def test_h_raised_state_reached(risk_session):
    """六图内容前提：RAISED 可见窗口内必须完成连拍（轮询重试超时即 FAIL，禁盲等假绿）。"""
    assert risk_session["first_frame"], "首帧未到达"
    assert risk_session["perception_ready"], "感知流数据未涌现"
    assert risk_session["behavior_ready"], "行为时间线数据未涌现"
    assert risk_session["raised_ready"], (
        f"RAISED 可见窗口未在 {RAISED_TIMEOUT_MS}ms 内捕获；诊断="
        f"{risk_session.get('diagnostics')}"
    )


def test_h_six_screenshots_captured(risk_session):
    """固定六张已落盘：文件存在 + 尺寸非零 + md5 manifest 入库。"""
    assert risk_session["raised_ready"], "前置 RAISED 未达成，未执行拍摄"
    rows = risk_session["rows"]
    ...
    assert Path(risk_session["manifest"]).exists(), "MANIFEST.md 未落盘"


def test_h_console_page_error_zero(risk_session):
    """console error / pageerror = 0（已知豁免子串过滤；豁免清单只增不减）。"""
    diag = f"4xx responses: {risk_session['responses_4xx'][:8]}"

    def _exempt(msg: str) -> bool:
        return any(pat in msg for pat in KNOWN_CONSOLE_EXEMPT)

    console_bad = [m for m in risk_session["console"] if not _exempt(m)]
    page_bad = [m for m in risk_session["pageerrors"] if not _exempt(m)]
