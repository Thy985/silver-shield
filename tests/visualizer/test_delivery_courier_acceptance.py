"""Delivery Courier Normal Acceptance — single-scenario browser test.
...
Key design decisions:
    ...
    - Expected max risk level: LOW (system stays restrained, no upgrade) — verified via dom lrk.level
    - Daytime + single visit means repeat_visit / OddHour rules do NOT trigger
...
OBSERVE_MS = _CONTRACT.observe_times.get("courier_observe", 120_000)
POLL_MS = 1_000
...
def acceptance():
    ...
    with sync_playwright() as p:
        ...
        data["cctv"] = _observe_phase(page, "courier_observe", OBSERVE_MS, need_source_switch=True)
