...
def test_live_stream_js_delta_renders_and_dedups():
    ...
    harness = textwrap.dedent(
        ...
        global.setInterval = function () { return -1; };  // 防止 Node 定时器阻塞进程退出
...
def test_live_stream_js_perception_stream_three_layers():
    """LP-3（Node vm）：感知流三层结构（CURRENT STATE + RECENT CHANGES + HISTORY）。

    验证：
    - ps-state 容器存在且包含 person/audio/risk 子项；
    - ps-recent 容器存在且包含空态占位符；
    - ps-history details 存在；
    - perception_delta → person 持续在场显示 + 首次出现条目；
    - risk_delta raised → risk 显示 + 风险条目推入 RECENT；
    - risk_delta cleared → risk 隐藏 + 解除条目推入 RECENT。
    """
    import subprocess
    import tempfile
    ...
    harness = textwrap.dedent(
        ...
        function makeEl(tag, id) {
          var el = { tag: tag || 'div', attrs: {}, html: '', text: '', style: {},
            _children: [],
            classList: { add: function () {}, remove: function () {} },
            getAttribute: function (k) { return this.attrs[k] != null ? this.attrs[k] : null; },
            setAttribute: function (k, v) { this.attrs[k] = String(v); },
        ...
        var els = {};
        var psState = makeEl('div', 'ps-state-live1');
        var psPerson = makeEl('div', 'ps-person-live1');
        psPerson._label = makeEl('span');
        psPerson.querySelector = function (sel) { if (sel === '.ps-label') return psPerson._label; return null; };
        psState.appendChild(psPerson);
        var psAudio = makeEl('div', 'ps-audio-live1');
        psAudio._label = makeEl('span');
        psAudio.querySelector = function (sel) { if (sel === '.ps-label') return psAudio._label; return null; };
        psState.appendChild(psAudio);
        var psRisk = makeEl('div', 'ps-risk-live1');
        psRisk._label = makeEl('span');
        psRisk.classList = {
          add: function (c) { psRisk._classes = psRisk._classes || {}; psRisk._classes[c] = true; },
          remove: function (c) { if (psRisk._classes) delete psRisk._classes[c]; },
          contains: function (c) { return psRisk._classes && !!psRisk._classes[c]; },
          toggle: function (c, force) {
            if (force === undefined) force = !psRisk._classes[c];
            if (force) psRisk.classList.add(c); else psRisk.classList.remove(c);
          }
        };
        psRisk.querySelector = function (sel) { if (sel === '.ps-label') return psRisk._label; return null; };
        psState.appendChild(psRisk);
        var psRecent = makeEl('div', 'ps-recent-live1');
        var psRecentEmpty = makeEl('div', 'ps-recent-empty-live1');
        psRecentEmpty.style.display = '';
        psRecent.appendChild(psRecentEmpty);
        var psHistory = makeEl('details', 'ps-history-live1');
        psHistory.style.display = 'none';
        var psHistoryCount = makeEl('span', 'ps-history-count-live1');
        var psHistoryList = makeEl('div', 'ps-history-list-live1');
        psHistory.appendChild(psHistoryCount);
        psHistory.appendChild(psHistoryList);

        var psContainer = makeEl('section');
        psContainer.appendChild(psState);
        psContainer.appendChild(psRecent);
        psContainer.appendChild(psHistory);
        els['perception-stream-live1'] = psContainer;

        const lp = makeEl('section', 'live-perception-live1');
        lp.attrs['data-scenario'] = 'live1';
        // lp 是 _applyPerceptionDelta 的入口，其 innerHTML 会被设置；
        // 但感知流子元素挂在 psContainer 下（不在 lp.innerHTML 路径上），
        // 需将 psContainer 挂到 lp 之外，避免 lp.innerHTML 覆盖感知流 DOM。
        // 策略：lp 的 innerHTML 用于视觉感知列表，感知流单独挂在 lp._stream 上。
        lp._stream = psContainer;
        const closurePanel = makeEl('div');
        closurePanel.getAttribute = function (k) { return k === 'data-ws-path' ? '/ws' : null; };

        [psState, psPerson, psAudio, psRisk, psRecent, psRecentEmpty, psHistory, psHistoryCount, psHistoryList, lp].forEach(function (e) {
          if (e.attrs.id) els[e.attrs.id] = e;
        });
        const doc = {
          querySelector: function (sel) {
            if (sel === '.live-perception') return lp;
        ...
        // ① 初始状态：三层容器存在
        results.initial = {
          stateExists: !!document.getElementById('ps-state-live1'),
          recentExists: !!document.getElementById('ps-recent-live1'),
          historyExists: !!document.getElementById('ps-history-live1'),
          emptyPlaceholderVisible: document.getElementById('ps-recent-empty-live1').style.display === '',
        };

        // ② perception_delta：1 人检测 → person 持续在场显示
        global._ws.onmessage({ data: JSON.stringify({
          type: 'perception_delta', frame_index: 10, case_time: 1.2,
          detections: [{ class: 'person', bbox: [10, 20, 200, 350], confidence: 0.88 }],
        })});
        var psAfterPerson = document.getElementById('ps-person-live1');
        var psAfterAudio = document.getElementById('ps-audio-live1');
        var psAfterRisk = document.getElementById('ps-risk-live1');
        results.personState = {
          personVisible: psAfterPerson.style.display !== 'none',
          personLabel: (psAfterPerson._label && psAfterPerson._label.text || '').indexOf('1 人') !== -1,
          audioHidden: psAfterAudio.style.display === 'none',
          riskHidden: psAfterRisk.style.display === 'none',
        };

        // ③ risk_delta raised：风险卡亮起 + 条目推入 RECENT
        global._ws.onmessage({ data: JSON.stringify({
          type: 'risk_delta', frame_index: 20, case_time: 2.5,
          risk_transition: 'raised', risk_levels: ['LOW'],
          reason_summary: ['未在白名单'], recommended_actions: ['MONITOR'],
        })});
        var psAfterRiskRaise = document.getElementById('ps-risk-live1');
        results.riskRaised = {
          riskVisible: psAfterRiskRaise.style.display !== 'none',
          riskLabel: (psAfterRiskRaise._label && psAfterRiskRaise._label.text || '').indexOf('LOW') !== -1,
        };

        // ④ risk_delta cleared：risk 隐藏 + 解除条目
        global._ws.onmessage({ data: JSON.stringify({
          type: 'risk_delta', frame_index: 30, case_time: 3.75,
          risk_transition: 'cleared', risk_levels: [],
          reason_summary: [], recommended_actions: [],
        })});
        var psAfterClear = document.getElementById('ps-risk-live1');
        results.riskCleared = {
          riskHidden: psAfterClear.style.display === 'none',
        };

        // ⑤ DOM 最终检查：RECENT CHANGES 包含首次出现、风险上升、风险解除
        var recentHtml = document.getElementById('ps-recent-live1').innerHTML;
        results.domCheck = {
          hasPersonEntry: recentHtml.indexOf('首次出现') !== -1,
          hasRiskRaised: recentHtml.indexOf('观察 → 关注') !== -1,
          hasRiskCleared: recentHtml.indexOf('关注 → 解除') !== -1,
        };

        var ok = Object.values(results).every(function (r) {
          return Object.values(r).every(function (v) { return v === true; });
        });
        console.log(ok ? 'PERCEPTION_STREAM_OK' : JSON.stringify(results, null, 2));
        process.exit(ok ? 0 : 1);
        """
    )
    with tempfile.NamedTemporaryFile("w", suffix=".js", delete=False, encoding="utf-8") as f:
    ...
    assert r.returncode == 0, f"感知流三层结构测试失败\nSTDOUT: {r.stdout}\nSTDERR: {r.stderr}"
    assert "PERCEPTION_STREAM_OK" in r.stdout
