...
def test_same_person_across_frames_is_one_active():
    ...
    times = [datetime(2026, 7, 19, 10, 0, i, tzinfo=UTC) for i in range(3)]
...
def test_visitor_leaves_after_absence_gap():
    ...
    times = [
        ...
        datetime(2026, 7, 19, 10, 0, 3, tzinfo=UTC),  # absence=2 < 5 → 仍 active
        datetime(2026, 7, 19, 10, 0, 10, tzinfo=UTC),  # absence=9 >= 5 → left
    ]
...
def test_revisit_keeps_frame_count_and_sets_active():
    ...
    times = [
        datetime(2026, 7, 19, 10, 0, 0, tzinfo=UTC),
        datetime(2026, 7, 19, 10, 0, 10, tzinfo=UTC),
        datetime(2026, 7, 19, 10, 0, 20, tzinfo=UTC),
    ]
...
def test_multiple_visitors_tracked_independently():
    times = [
        datetime(2026, 7, 19, 10, 0, 0, tzinfo=UTC),
        datetime(2026, 7, 19, 10, 0, 1, tzinfo=UTC),
        datetime(2026, 7, 19, 10, 0, 2, tzinfo=UTC),
    ]
...
def test_caviar_one_stop_enter_detects_person():
    ...
    _det, results = _run_detector_on_sequence(frames)
...
def test_caviar_meet_walk_together_detects_multi_person():
    ...
    _det, results = _run_detector_on_sequence(frames, conf_threshold=0.10, enable_track=False)
...
def test_caviar_meet_walk_together_track_id_unique_in_frame():
    ...
    if frames is None:
        pytest.skip("CAVIAR fixture 缺失")
    _det, results = _run_detector_on_sequence(frames, conf_threshold=0.10, enable_track=True)
    for r in results:
